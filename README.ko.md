...
"검색 엔드포인트"가 무슨 말인지 몰라도 괜찮습니다 — 명령은 그냥 평소 말이면 됩니다. 상황은 둘로 나뉩니다:

**프로젝트를 처음 시작할 때** — 아직 아무것도 없을 때. 목표를 말하고 "실제로 작동하는지 확인까지"를 붙이면 됩니다:

```text
아발론으로 해줘 — 우리 카페 홈페이지를 처음부터 만들어줘.
메뉴 소개, 오시는 길, '문의하기' 폼. 셋 다 실제로 작동하는지 확인까지.
```

**하던 프로젝트에 뭔가 더할 때** — 그 프로젝트 폴더에서 클로드코드를 열고 부탁하면 됩니다. 에이전트는 손대기 전에 이미 있는 것부터 실측(`scaffold`)합니다:

```text
아발론으로 해줘 — 만들던 홈페이지에 '예약하기' 기능 넣어줘.
예약을 남기면 목록에 실제로 쌓이는지 확인까지.
```

이런 식으로도 씁니다:

- 되돌리기 무서운 수정: *"아발론으로 — 결제 부분 고쳐줘. 되돌릴 수 없는 단계는 하기 전에 나한테 물어봐."*
- 자는 동안 맡길 때: *"아발론으로 — 이 목록 밤새 처리해줘. 기준 미달이면 대충 넘어가지 말고 멈춰."*

숫자·게이트·그래프 설계는 에이전트의 일입니다. 대신 에이전트는 **일을 시작하기 전에** 그 기준을 알아들을 수 있는 말로 먼저 보여줘야 합니다 — "보내기를 누르면 저장된 문의가 1건 늘어야 함, 실패한 검사 0개" 같은 식으로. 당신이 할 일은 세 가지뿐입니다:

1. 시작 전에 나온 기준 목록을 읽고 "그래, 그게 '완성'이 맞아"라고 답한다.
2. 중간에 "이 단계는 되돌릴 수 없는데 진행할까요?"라고 물으면 답한다.
3. 끝나면 신호등을 읽는다 — 초록 넷이면 끝난 것이고, `abandoned`에 뭔가 있으면 "이 부분은 못 했고, 증거는 이것"이라는 뜻입니다.

당신이 절대 만질 일 없는 것들: `graph.json`(에이전트가 씀), 명령어(에이전트가 침), 임계값(에이전트가 제안 — 당신은 그게 당신이 생각하는 '완성'이 맞는지만 답하면 됩니다).
