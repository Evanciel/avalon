...
영문 정본: [SKILL.md](SKILL.md) · 도구 메시지 기본 언어는 영어다 — `AVALON_LANG=ko` 로 한국어.
...
- 훅 항목에 선택적으로 `"probe": "<명령>"`을 단다 — 일부러 고장난 입력을 겨눠 **반드시 exit≠0**이
  나와야 하는 반증 명령이다. check가 **실패할 줄 안다**는 것을 증명한다. 실패할 줄 모르는
  오라클(프로브가 exit 0)은 설치자가 장식으로 보고 거부한다.
...
node tools/run.mjs graph.json verify    # 원장 해시 체인 검증 (변조·절단 탐지)
```

사람/에이전트는 **측정값만** 넣는다. 프론티어 밖 노드는 start가 거부되고, 이번 방문에 안 잰
필드는 done이 거부되며, 모든 측정은 원장에 append-only로 남는다. 원장 각 줄은 **해시 체인**으로
엮인다(`h`/`prev` + 상태의 `ledger_head`) — 과거 줄을 고치거나 지우거나 순서를 바꾸면 모든
명령이 거부한다. 그래프 해시가 바뀌면 상태가 STALE로 표시된다 — 조용히 이어가지 않는다.
...
node tools/install-hooks.mjs graph.json build/hooks.json --status # 읽기 전용 진단
```

설치자가 지키는 경계 (install.selftest.mjs 가 고정):
...
- 승인 시점 명세의 바이트 해시를 설치 명령에 박제(`--approved`) — 이후 hooks.json 이 어떻게 바뀌든
  게이트는 **실행 없이** 차단한다. 재승인 전에는 새 check 가 돌 수 없다.
- 선언된 프로브는 설치 시점에 exit≠0 이어야 한다 — 실패할 줄 모르는 오라클은 장식으로 거부한다.
...
check가 게이트의 제목과 같은 뜻인지는 증명하지 못한다. 프로브는 이 간극을 좁히지만(오라클이
실패할 줄 안다는 것까지는 증명한다) 닫지는 못한다. 재지 않은 것을 잰 것처럼 쓰지 마라.

## 검증

```bash
npm test   # tools/test.mjs (스키마·컴파일러·실행 의미론) + run.selftest.mjs (러너 거부 방어벽) + install.selftest.mjs (설치자·게이트 방어벽)
```
