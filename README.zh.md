...
装好技能后([安装](#安装)),整个调用就是一句大白话:

```text
用 avalon 来推进 — 给 my-api 加一个搜索端点,测试必须全绿。
```

措辞随意("按 avalon 的流程"、"走图流程"…) — 技能的触发词就是这个名字。接下来无需再指挥:
...
2. 逐节点干活、提交测量值,工具回答通过或不通过。
3. 在不可逆节点停下来问你 — 这次停顿不是客气,是编译进产物里的。
4. 最终报告带着 `completed` 和 `abandoned[]` 证据列表 — 智能体无法伪造的两栏。

你的实际角色:第 1 步把门的设计读一遍(阈值之争归你),回答审批问题,最后读四个数字。

### 不是开发者也能用

要是"搜索端点"这词你根本没听懂,没关系 — 命令就是日常大白话。分两种情况:

**从零开始一个项目时** — 现在什么都还没有。说出目标,再加一句"确认真的能用":
...
**给做到一半的项目加东西时** — 在那个项目的文件夹里打开 Claude Code 直接说就行。智能体动手之前会先实测已有的东西(`scaffold`):
...
- 不敢乱动的修改:*"用 avalon — 修一下支付部分。凡是撤销不了的步骤,动手前先问我。"*
- 睡觉时交给它:*"用 avalon — 今晚把这个清单处理完。有门不过就停下,别糊弄过去。"*

数字、门、图的设计都是智能体的活。作为交换,它必须**在开工之前**用你听得懂的话把标准亮出来 — 比如"点发送后,已保存的留言必须多一条;失败的检查为零"。你要做的只有三件事:

1. 开跑前读一遍标准清单,回答"对,'做完'就是这个意思"。
2. 中途它问"这一步无法撤销,继续吗?"时,回答它。
3. 结束后看红绿灯 — 四个绿灯就是完成;`abandoned` 里有东西,意思是"这部分没做成,证据在此"。

你永远不用碰的东西:`graph.json`(智能体写)、命令(智能体敲)、阈值(智能体提议 — 你只需回答那是不是你心里的"做完")。

### 在 GPT 或任何其他智能体里

框架里没有任何 Claude 专属的东西 — 工具就是 6 个普通 Node CLI,没有 API 密钥,不调用任何厂商。任何能跑 shell 命令的智能体,都可以直接指给它流程:
...
每个智能体受到的待遇都一样:验证器对它的图草稿行使否决权,运行器拒绝它抄近道,编译出的 `completed` 无视它的意见。就算完全没有 AI,运行器 CLI 也照样能用 — 当作一份给人类的纪律清单。
...
<img src="images/stop-hook.svg" alt="智能体要结束回合时,hooks-gate 跑一遍所有 check:全绿则回合结束,有红则 exit 2 拦截,并把不合格的门反馈给模型" width="100%" />
